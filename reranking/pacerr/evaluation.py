...
class CERerankingEvaluator_ndcg:
    """
    ...
    def __init__(self, samples, name: str = "", write_csv: bool = True):
        ...
        self.csv_headers = ["epoch", "steps", "ndcg"]
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for i, instance in enumerate(self.samples):
            ...
            # for ir measures
            qrels[f"Q_{i}"] = {f"D_{k}": v for k, v in enumerate(is_relevant)}
        ...
        if output_path is not None and self.write_csv:
            ...
            with open(csv_path, mode="w" if output_file_exists else "w", encoding="utf-8") as f:
                ...
                writer.writerow([epoch, steps, aggregated_ndcg])

        return aggregated_ndcg


class CERerankingEvaluator_MAP:
    """
    This class evaluates a CrossEncoder model for the task of re-ranking.

    Given a query and a list of documents, it computes the score [query, doc_i] for all possible
    documents and sorts them in decreasing order. Then, map@10 is compute to measure the quality of the ranking.
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for instance in self.samples:
            ...
            k = len(positive)
            n_hits = 0
            for rank, index in enumerate(pred_scores_argsort[:k]):
                if is_relevant[index]:
                    n_hits += 1
                    map_score += n_hits / (rank + 1)

            all_map_scores.append(map_score / k)
