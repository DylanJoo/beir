...
if __name__ == '__main__':
    ...
    dataset_inpars = load_dataset('json', data_files=args.pseudo_queries)['train']
    ...
    train_samples = []
    dev_samples = []
    for i in range(len(dataset_inpars)):
        document = dataset_inpars[i]['document']

        #### Filtering
        # pairs = filter_fn(pseudo_queries[docid], **filter_args)
        # queries, scores = map(list, (list(zip(*pairs))) )
        queries, scores = dataset_inpars[i]['generated_query'], dataset_inpars[i]['relevance_scores']
